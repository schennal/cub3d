...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/06/29 13:39:20 by schennal          #+#    #+#             */
/*   Updated: 2024/06/29 13:39:21 by schennal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (map_tab[i] != NULL)
	{
		...
		while (map_tab[i][j])
		{
			...
			if (!(ft_strchr("10NSEW", map_tab[i][j])))
				return (print_err(parse->ip_map.path, "ERR_INV_LETTER", FAILURE));
			if (ft_strchr("NSEW", map_tab[i][j]) && parse->pdir != '0')
				return (print_err(parse->ip_map.path, "ERR_NUM_PLAYER", FAILURE));
			if (ft_strchr("NSEW", map_tab[i][j]) && parse->pdir == '0')
	...
	if (ft_isprint(map[i][j]) && !ft_isdigit(map[i][j]))
	{
			...
			&& !ft_isdigit(map[i][j]))
		{
			if (fill_dir_tex(parse, map[i], j) == ERR)
				return (print_err(parse->ip_map.path, "bad texture", FAILURE));
			return (BR);
		}
		else
		{
