...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/06/29 13:39:07 by schennal          #+#    #+#             */
/*   Updated: 2024/06/29 13:39:08 by schennal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static size_t	find_biggest_len(t_map *map, int i)
{
	size_t	biggest_len;
...
{
	parse->ip_map.height = count_map_lines(parse, file, i);
	if (parse->ip_map.height == 0)
		return (print_err(NULL, "NO_MAP_FOUND", FAILURE));
	parse->map = ft_calloc(sizeof(char *), (parse->ip_map.height + 1));
	if (!parse->map)
		return (print_err(NULL, "Could not allocate memory", FAILURE));
	if (fill_map_tab(&parse->ip_map, parse->map, i) == FAILURE)
		return (FAILURE);
