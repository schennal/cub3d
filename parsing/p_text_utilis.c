...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/06/29 13:39:48 by schennal          #+#    #+#             */
/*   Updated: 2024/06/29 13:39:50 by schennal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	fill_color_textures(t_parse *parse, char *line, int j)
{
	if (line[j + 1] && ft_isprint(line[j + 1]))
		return (print_err(parse->ip_map.path, "Invalid color", ERR));
	if (!parse->ceiling && line[j] == 'C')
	{
		parse->ceiling = set_rgb_colors(line + j + 1);
		if (parse->ceiling == 0)
			return (print_err(parse->ip_map.path, "Invalid ceiling color",
					ERR));
	}
	else if (!parse->floor && line[j] == 'F')
	{
		...
		if (parse->floor == 0)
			return (print_err(parse->ip_map.path, "Invalid floor color", ERR));
	}
	else
		return (print_err(parse->ip_map.path, "Invalid Color", ERR));
	return (SUCCESS);
